...
| 登录池 | 可见的人工登录窗口 | 检测到需要登录时后台预热 | 30 分钟 | 按空闲时间回收 |
...
Runner 使用彼此独立的自动检查和人工登录工作通道。当前检查可以继续完成，人工登录无需在检查队列后等待；登录活动期间后台通道不会再领取下一项检查。

自动检查可在设置页配置 1–3 路并发，默认保持 1 路。多个槽位共享同一个无头 Edge，并使用彼此隔离的 BrowserContext；手动、批量和登录后重查优先于定时任务。

自动检查发现登录页后，Runner 会在后台启动登录专用 Edge，但暂不创建可见页面。用户随后点击登录时会打开可见页面。完成或取消后窗口最小化，并在 30 分钟内复用同一个 Context；多个过期网站可以在应用提示后继续使用同一个窗口处理。

如果用户没有继续登录，工作区会在 30 分钟空闲期后自动退出并清理临时 Profile。长期加密状态仍只保存目标站点的数据，跨域认证状态不会写入数据库。

自动检查领取浏览器状态时会同时记录版本号。若人工登录在检查期间保存了更新状态，较早开始的检查不会覆盖新的 Cookie 和站点存储。
...
桌面版使用 Windows Job Object 管理后台进程：
